...
let assoc_list = function
  | Sexplib.Sexp.List ls ->
     List.map (function | Sexplib.Sexp.List (Sexplib.Sexp.Atom x :: xs ) -> Some (x,xs) | _ -> None) ls
     |> List.fold_left (fun acc x -> match x with | Some y -> y::acc | None -> acc) [] |> List.rev |> Result.ok
  | _ -> Result.error (`Msg "Expecting list in assoc_list")

let string_of_atom = function | [Sexplib.Sexp.Atom x] -> Ok x | _ -> Error (`Msg "Expecting singleton atom in string_of_atom")

let assoc name l =
  try Ok (List.assoc name l) with _ -> Error (`Msg (Printf.sprintf "Expecting key '%s' in assoc - keys are: [%s]" name (String.concat ", " (List.map fst l))))
...
let process_wrapped_library d =
  ...
  let modules = List.fold_right (fun l acc ->
    let r = l >>= assoc "name" >>= string_of_atom in
    match r with
    | Ok x -> x::acc
    | Error _ -> acc) modules [] in
...
let process_library : Sexplib.Sexp.t -> (Library.t, [>`Msg of string]) result  = fun s ->
  assoc_list s >>= fun l ->
  assoc "name" l >>= string_of_atom >>= fun name ->
  assoc "modules" l >>= fun m ->
  (match m with
   | [Sexplib.Sexp.List x] -> Ok x
   | _ -> Error (`Msg (Printf.sprintf "modules wasn't a singleton list"))) >>= fun m ->
  (match m with
   ...
   | Sexplib.Sexp.Atom x :: _ -> Error (`Msg (Format.sprintf "Unexpected library type: %s\n%!" x))
   | _ -> Error (`Msg (Format.sprintf "Very unexpected library structure"))) >>= fun ty ->
  let dependencies = match assoc "requires" l with
    | Ok deps ->
      List.fold_right (fun x acc -> match x with | Sexplib.Sexp.Atom y -> y::acc | _ -> acc) deps []
    | Error _ ->
      []
  in
...
let process_dune_package : Sexplib.Sexp.t -> (t, [> `Msg of string]) result  = fun s ->
  assoc_list s >>= fun l ->
  assoc "name" l >>= string_of_atom >>= fun name ->
  let version = match assoc "version" l >>= string_of_atom with Ok x -> Some x | Error _ -> None in
  List.fold_right (fun (x,l) acc ->
    acc >>= fun acc ->
    match x,l with
    | "library", l -> begin
    ...
    | _ -> Ok acc) l (Ok []) >>= fun libraries ->
  
  Ok { name; version; libraries}
...
let sexp_of_file p =
  ...
  with e ->
    Error (`Msg (Printf.sprintf "Error in process_file: %s" (Printexc.to_string e)))
...
let find package =
  ...
  Bos.OS.Dir.fold_contents ~dotfiles:true
      (fun p acc ->
        ...
        if name = Fpath.v "dune-package"
        then begin
          Ok p
        end else acc) (Error (`Msg "Missing")) path |> Result.join
