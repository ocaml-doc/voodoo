...
let mk ?(attr = []) desc = { Ast.Raw.bl_desc = desc; bl_attributes = attr }
...
module Pre = struct
  ...
  and t = { blocks : Raw.block list; next : container }
  ...
  let rec close link_defs { blocks; next } =
    ...
    match next with
    | Rblockquote state -> mk (Raw.Blockquote (finish state)) :: blocks
