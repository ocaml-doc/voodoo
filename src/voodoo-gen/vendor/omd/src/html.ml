...
and inline { il_desc; il_attributes = attr } =
  ...
  | Text t -> text t
  | Emph il -> elt Inline "em" attr (Some (inline il))
  | Strong il -> elt Inline "strong" attr (Some (inline il))
  | Code s -> elt Inline "code" attr (Some (text s))
  | Hard_break -> concat (elt Inline "br" attr None) nl
  | Soft_break -> nl
  | Html body -> raw body
  | Link { label; destination; title } -> url label destination title attr
  | Image { label; destination; title } -> img label destination title attr

let rec block { bl_desc; bl_attributes = attr } =
  match bl_desc with
  | Blockquote q ->
      elt Block "blockquote" attr (Some (concat nl (concat_map block q)))
  | Paragraph md -> elt Block "p" attr (Some (inline md))
  | List (ty, sp, bl) ->
  ...
  | Thematic_break -> elt Block "hr" attr None
