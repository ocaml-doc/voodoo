...
let read_file dir f =
  try
    ...
    let rec inner lines =
      try inner @@ input_line ic :: lines with End_of_file -> List.rev lines
    in
    (f, inner [])
  with Sys_error _ -> (f, [])
...
let parse_file (filename, lines) =
  ...
  | line :: _ -> (
      ...
      let pkg =
        ...
        | Some p -> p
        | None -> failwith "Failed to parse package name"
      in
      let filter =
        [
          "base-unix";
        ...
        @ if pkg.Opam.name = "ocaml-base-compiler" then [ "ocaml" ] else []
      in
      let atoms =
        List.filter (fun atom -> not @@ List.mem atom.Opam.name filter) atoms
      in
      let packages = Universe.S.of_list atoms in
      match Opam.of_string filename with
      | Some pkg -> Some (pkg, Universe.of_packages packages)
      | None ->
          Format.eprintf "bad string: %s!\n%!" filename;
          None)
  | _ -> None
...
let cache () =
  ...
  if false then
    let dune_cache =
      Obuilder_spec.Cache.v "dune-cache" ~target:"/home/opam/.config/dune"
    in
...
let ocluster_build ~cap ~base_obuilder ~stdout ~stderr c =
  ...
  let action =
    Cluster_api.Submission.obuilder_build (obuilder_to_string obuilder_content)
  in
  Capnp_rpc_lwt.Capability.with_ref
    (Cluster_api.Submission.submit submission_service ~urgent:false
       ~pool:"linux-x86_64" ~action ~cache_hint:"")
  @@ fun ticket ->
  Capnp_rpc_lwt.Capability.with_ref (Cluster_api.Ticket.job ticket)
  @@ fun job ->
  ...
  let proc =
    let rec tail job start =
      ...
      | Error (`Capnp e) ->
          Lwt_io.write stderr
            (Fmt.str "Error tailing logs: %a" Capnp_rpc.Error.pp e)
  ...
  let timeout =
    ...
    Lwt_io.write_line stderr
      ("Command '" ^ c ^ "' timed-out (" ^ string_of_int hours ^ " hours).")
...
let exec_out ~fexec ~fout =
  ...
  let proc =
    Lwt.finalize (fun () -> fexec ~stdout) (fun () -> Lwt_io.close stdout)
  in
  fout ~stdin >>= fun res ->
...
let ocluster_build_str ~cap ~base_obuilder ~stderr ~default c =
  ...
  exec_out ~fout:aux ~fexec:(fun ~stdout ->
      ocluster_build ~cap ~base_obuilder ~stdout ~stderr
        ("echo @@@ && " ^ c ^ " && echo @@@"))
  >>= function
  | Ok (), r -> Lwt.return r
  | Error (), _ -> (
      ...
      | None ->
          ...
          (* TODO: Replace this with "send message to debug slack webhook" *)
      | Some v -> Lwt.return v)
...
let failure_kind logfile =
  Lwt_io.with_file ~mode:Lwt_io.Input (Fpath.to_string logfile) (fun ic ->
      let rec lookup res =
        ...
        | Some
            "This package failed and has been disabled for CI using the \
             'x-ci-accept-failures' field." ->
            lookup `AcceptFailures
...
let with_test pkg = if false then with_test pkg else ""

let run_script pkg universe =
  let packages =
    Universe.M.fold
      (fun name version s -> Opam.{ name; version } :: s)
      universe.Universe.packages [ pkg ]
  in
  let packages_str =
    String.concat " "
      (List.map (fun pkg -> Format.asprintf "%a" Opam.pp_package pkg) packages)
  in
  {|
...
let run_job ~cap ~stderr ~base_obuilder ~num ~pool pkg universe =
  Lwt_pool.use pool (fun () ->
      Lwt_io.write_line stderr
        (Format.asprintf "[%s] Checking %a..." num Opam.pp_package pkg)
      >>= fun () ->
      let logfile = Fpath.v (Format.asprintf "%a.log" Opam.pp_package pkg) in
      Lwt_io.with_file
        ~flags:Unix.[ O_WRONLY; O_CREAT; O_TRUNC ]
        ~perm:0o640 ~mode:Lwt_io.Output (Fpath.to_string logfile)
        (fun stdout ->
          ocluster_build ~cap ~base_obuilder ~stdout ~stderr
            (run_script pkg universe))
      >>= function
      | Ok () ->
          Lwt_io.write_line stderr ("[" ^ num ^ "] succeeded.") >>= fun () ->
          Lwt_io.open_file ~mode:Output
            (Format.asprintf "%a.success" Opam.pp_package pkg)
          >>= fun c ->
          Lwt_io.write_line c "success" >>= fun () -> Lwt_io.close c
      | Error () -> (
          failure_kind logfile >>= function
          | `Partial ->
              Lwt_io.write_line stderr
                ("[" ^ num ^ "] finished with a partial failure.")
          | `Failure -> Lwt_io.write_line stderr ("[" ^ num ^ "] failed.")
          | `NotAvailable ->
              Lwt_io.write_line stderr
                ("[" ^ num ^ "] finished with not available.")
          | `Other | `AcceptFailures | `Timeout ->
              Lwt_io.write_line stderr
                ("[" ^ num ^ "] finished with an internal failure.")))
...
let get_obuilder from =
  ...
  stage ~from
    ([
       ...
       run ~network
         "python3 -m pip install git+https://github.com/s3tools/s3cmd.git";
       run
         "echo running again and again and again and again and again and again";
       run ~network "git -C opam-repository pull origin master";
       (* "&& git -C opam-repository checkout %s" (Filename.quote opam_repo_commit); *)
     ]
    @ (if false then
       [
         run ~network
           "git -C opam-repository pull \
            git://github.com/kit-ty-kate/opam-repository.git opam-health-check";
       ]
      else [])
    @ [
        ...
        run ~network ~cache
          "opam pin add -y git://github.com/jonludlam/voodoo-prep";
...
let run_jobs ~cap ~stderr ~pool pkgs =
  ...
  List.map
    (fun (pkg, universe) ->
      ...
      Format.eprintf "pkg: %a universe: %a" Opam.pp_package pkg Universe.pp
        universe;
      ...
      | Some v ->
          ...
          let docker_tag =
            Printf.sprintf "ocaml/opam:debian-unstable-ocaml-%s" v
          in
...
let get_cap ~stderr =
  ...
  let cap_file = home ^ "/ocluster.cap" in
  (* TODO: fix that *)
...
let wait_current_run_to_finish =
  let rec loop () =
    if is_running () then Lwt_unix.sleep 1. >>= loop else Lwt.return_unit
  in
  loop
...
let rec drop n l =
  match (n, l) with 0, x -> x | _, [] -> [] | n, _ :: xs -> drop (n - 1) xs

let run pkgs =
  let pool = Lwt_pool.create 100 (fun () -> Lwt.return ()) in

  let stderr = Lwt_io.stderr in
  get_cap ~stderr >>= fun cap -> run_jobs ~cap ~stderr ~pool pkgs
